...
- Le code serveur, sous `/dispatcher`, s'appuie désormais sur le framework Spring. Cela permet une déclaration plus légère des Exchanges, 
...
- [x] Prise en charge par Spring de la connexion TLS du hub via [`application.properties`](../dispatcher/src/main/resources/application.properties)
- [x] Déclaration des files et exchanges RabbitMQ prise en charge par Spring via [`AmqpConfiguration`](../dispatcher/src/main/java/com/hubsante/dispatcher/config/AmqpConfiguration.java)
- [x] Le dossier [`models/`](../models) exploite le fichier de spécifications AsyncAPI pour générer les classes correspondantes
- [x] Les classes sont ensuite intégrés dans [`com.hubsante.message`](../dispatcher/src/main/java/com/hubsante/message) et peuvent être utilisées facilement par les autres fichiers
...
### Docker (RabbitMQ & Hub)
```bash
## Reconstruire les images du Hub (se positionner dans le répertoire dispatcher)
