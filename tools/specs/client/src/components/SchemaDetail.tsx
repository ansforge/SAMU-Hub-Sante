...
function fieldType(prop: JsonSchemaProperty, definitions: JsonSchemaDefinitions): string {
  const resolved = resolveRef(prop, definitions);

  if (resolved.type === "array") {
    const itemType = resolved.items ? fieldType(resolved.items, definitions) : "?";
...
function FieldDescription({ prop }: { prop: JsonSchemaProperty }) {
  ...
  return (
    <div className="text-sm text-muted-foreground">
      {prop.description}
      {prop.enum && <div className="mt-0.5 text-xs">enum : {prop.enum.join(", ")}</div>}
    </div>
  );
}
  ...
  return (
    ...
    >
      {Object.entries(properties).map(([name, prop]) => {
        ...
        if (!nested) {
          return (
            <div key={name} className="not-last:border-b px-3 py-2.5">
              <FieldHeader name={name} prop={prop} required={isRequired} definitions={definitions} />
...
export function SchemaDetail({ schema }: SchemaDetailProps) {
  const [expandSignal, setExpandSignal] = useState<ExpandSignal>({ key: 0, expand: false });
