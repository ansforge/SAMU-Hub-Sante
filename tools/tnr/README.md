...
│   ├── Utils.java            ← utilitaires (sanitisation des clientId)
│   └── dto/
...
protected void deliverCallback(String consumerTag, Delivery delivery) throws IOException {
    ...
    MessageDTO message = new MessageDTO(this.vhost, this.queueName, objectNode);
    inbox.add(message);
}
```
...
Étend `AMQPTestSupport` et hérite des producteurs/consommateurs. Chaque méthode de test :

1. Lit le fichier JSON EDXL depuis le chemin configuré dans `.env` (`EDXL_EXAMPLE_FILE_PATH_V1` ou `EDXL_EXAMPLE_FILE_PATH_V3` selon le cas)
2. Génère un `distributionID` unique (`clientId + "_" + UUID.randomUUID()`)
...
6. Vérifie avec des assertions que le `MessageDTO` reçu contient le bon `distributionID`, le bon `vhost` et la bonne `queue`

#### Cas de test

| Test | Description |
|---|---|
| `messagePublishedByProducerIsReceivedByConsumer` | SAMU1-V1 → SAMU2-V1 (même version, vhost `15-15_v1.5`) |
| `messageFromV3ToV1` | SAMU-V3 → SAMU1-V1 (inter-versions, v3 → v1) |
| `messageFromV1ToV3` | SAMU1-V1 → SAMU-V3 (inter-versions, v1 → v3) |
| `messageFromV1ToNexsis` | SAMU1-V1 → SDIS-Z (intégration Nexsis, vhost `15-nexsis_v1.9`) |
| `messagePublishedBy2ProducersCanBeReceivedInMisorder` | 2 messages publiés, réception attendue dans l'ordre inverse |
