    ...
    sections: [
      {
        items: [
          {
            label: `Résultats de la recette du cas de test: ${testCase.value.label}`,
            fontSize: 20,
            color: color.black,
          },
          {
            label: `Description: ${testCase.value.description}`,
            fontSize: 16,
            color: color.black,
          },
          {
          ...
          {
            label: `Vhost: ${store.selectedVhost.vhost}`,
            fontSize: 16,
            color: color.black,
          },
          {
            label: `Modèle: ${store.selectedVhost.modelVersion}`,
            fontSize: 16,
            color: color.black,
          },
          {
            label: `Date: ${new Date().toLocaleString()}`,
            fontSize: 16,
            color: color.black,
          },
        ],
      ...
      ...testCase.value.steps.map((step) => {
        ...
        return {
          items: [
            ...
            {
              label: `Type : ${
                step.type === 'receive' ? 'Réception' : 'Envoi'
              }`,
              fontSize: 10,
              color: color.black,
            },
          ...
          counts: [
            {
              label: `${
                counts.total - counts.unreviewed
              } valeurs renseignés sur ${counts.total} :`,
              fontSize: 10,
              color: color.black,
            },
            {
              label: `  - correctes: ${counts.valid}`,
              fontSize: 10,
              color: color.green,
            },
            {
              label: `  - approximatives: ${counts.approximate}`,
              fontSize: 10,
              color: color.orange,
            },
            {
              label: `  - incorrectes: ${counts.invalid}`,
              fontSize: 10,
              color: color.red,
            },
          ],
