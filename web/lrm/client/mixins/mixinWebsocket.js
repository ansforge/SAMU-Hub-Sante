...
import { buildAck, sendMessage, getMessageType, getReadableMessageType, timeDisplayFormat } from '~/composables/messageUtils'
      ...
      if (this.$options.name === 'Demo' || this.$options.name === 'Testcase') {
        this.store.socket.addEventListener('message', (event) => {
          ...
          // Send back acks automatically to received messages
            if (getMessageType(message) !== 'ack' && message.routingKey.startsWith(this.store.user.clientId)) {
