  ...
  methods: {
      ...
      if (this.$options.name === "Demo" || this.$options.name === "Testcase") {
        this.socket.addEventListener("message", (event) => {
          ...
          if (this.autoAck) {
            ...
            if (
              this.getMessageType(message) !== "ack" &&
    ...
    buildMessage(innerMessage, distributionKind = "Report") {
      ...
      message.content[0].jsonContent.embeddedJsonContent.message.sender = {
        name,
        URI: `hubex:${this.user.clientId}`,
      };
      message.content[0].jsonContent.embeddedJsonContent.message.sentAt =
        sentAt;
      message.content[0].jsonContent.embeddedJsonContent.message.recipients = [
        {
          name: this.clientInfos(this.user.targetId).name,
          URI: `hubex:${targetId}`,
        },
      ];
      return message;
