...
public class ConversionUtilsTest {
    ...
    void testRequiresConversion() {
        try (MockedStatic<ConversionUtils> mockedConversionUtils =
            ...
            // List of test cases: requiresCisuConversion, requiresVersionConversion, (expected)
            // requiresConversion values
            List<Boolean[]> testCases =
                    Arrays.asList(
            ...
            mockedConversionUtils
                    .when(() -> ConversionUtils.requiresConversion(hubConfig, edxlMessage))
                    .thenCallRealMethod();
            ...
            for (int i = 0; i < testCases.size(); i++) {
                ...
                mockedConversionUtils
                        .when(() -> ConversionUtils.requiresCisuConversion(hubConfig, edxlMessage))
                        .thenReturn(testCase[0]);
                mockedConversionUtils
                        .when(
                                () ->
                                        ConversionUtils.requiresVersionConversion(
                                                hubConfig, edxlMessage))
                        .thenReturn(testCase[1]);
    ...
    void testRequiresVersionConversion(
        ...
        try (MockedStatic<MessageUtils> mockedMessageUtils = mockStatic(MessageUtils.class)) {
            ...
            assertEquals(
                    expected, ConversionUtils.requiresVersionConversion(hubConfig, edxlMessage));
        }
    }
    ...
    void testGetTargetVersion() {
        try (MockedStatic<MessageUtils> mockedMessageUtils = mockStatic(MessageUtils.class)) {
            ...
            mockedMessageUtils
                    .when(() -> MessageUtils.getRecipientID(edxlMessage))
                    .thenReturn("fr.health.samuEmpty");
            assertArrayEquals(
                    new String[] {}, ConversionUtils.getTargetVHosts(hubConfig, edxlMessage));
            ...
            mockedMessageUtils
                    .when(() -> MessageUtils.getRecipientID(edxlMessage))
                    .thenReturn("fr.fire.test");
            assertArrayEquals(
                    new String[] {"15-nexsis_v1.9"},
                    ConversionUtils.getTargetVHosts(hubConfig, edxlMessage));

            mockedMessageUtils
                    .when(() -> MessageUtils.getRecipientID(edxlMessage))
                    .thenReturn("fr.cisu.test");
            assertArrayEquals(
                    new String[] {"15-nexsis_v1.9"},
                    ConversionUtils.getTargetVHosts(hubConfig, edxlMessage));
            ...
            mockedMessageUtils
                    .when(() -> MessageUtils.getRecipientID(edxlMessage))
                    .thenReturn("fr.health.samuDirectCisu");
            assertArrayEquals(
                    new String[] {"15-nexsis_v1.9"},
                    ConversionUtils.getTargetVHosts(hubConfig, edxlMessage));
        }
    ...
    void testRequiresCisuConversion() {
        try (MockedStatic<ConversionUtils> mockedConversionUtils =
                mockStatic(ConversionUtils.class)) {
            // List of test cases: isCisuExchange, isConvertedModel, isDirectCisuForHealthActor,
            // expectedResult
            ...
            when(edxlMessage.getDescriptor().getExplicitAddress().getExplicitAddressValue())
                    .thenReturn("fr.fire.something");
            when(hubConfig.getVhost()).thenReturn("15-15_v2.1");
            ...
            for (int i = 0; i < testCases.size(); i++) {
                ...
                mockedConversionUtils
                        .when(() -> ConversionUtils.isOneCisuHubexInvolved(edxlMessage))
                        .thenReturn(testCase[0]);
                mockedConversionUtils
                        .when(
                                () ->
                                        ConversionUtils.isAlreadyCisuConverted(
                                                hubConfig.getVhost(),
                                                MessageUtils.getRecipientID(edxlMessage)))
                        .thenReturn(false);
                mockedConversionUtils
                        .when(
                                () ->
                                        ConversionUtils.isDirectCisuForHealthActor(
                                                hubConfig, edxlMessage))
                        .thenReturn(testCase[1]);
